...
describe('resolveAuthRedirect', () => {
  ...
  it('인증된 사용자가 관리자 회원가입 페이지에 접근하면 대시보드로 보낸다', () => {
    expect(resolveAuthRedirect('/admin/register', true)).toEqual({ pathname: '/dashboard' });
  });
  ...
  it('비인증 사용자는 관리자 회원가입 페이지에 직접 접근할 수 있다', () => {
    expect(resolveAuthRedirect('/admin/register', false)).toBeNull();
  });
});
