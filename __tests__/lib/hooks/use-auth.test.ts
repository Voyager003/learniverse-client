...
const { mockAdminLogin, mockAdminRegister } = vi.hoisted(() => ({
  mockAdminLogin: vi.fn(),
  mockAdminRegister: vi.fn(),
}));
...
describe('useAuth', () => {
  ...
  it('일반 회원가입 성공 시 사용자 정보를 가져와 스토어에 저장한다', async () => {
  ...
  it('관리자 회원가입은 토큰을 저장하지 않는다', async () => {
    mockAdminRegister.mockResolvedValueOnce({
      email: 'admin@example.com',
      role: 'admin',
    });
    ...
    await act(async () => {
      await result.current.registerAdmin({
        email: 'admin@example.com',
        password: 'pass123',
        name: '관리자',
      });
